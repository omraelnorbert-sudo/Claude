...
export default function LoginForm({ next }: { next: string }) {
  ...
  async function sendMagicLink(event: React.FormEvent) {
    ...
    setStatus("sent");
    setMessage(`Link an ${email} geschickt. Schau in dein Postfach.`);
  }
  ...
  return (
    <div className="auth-methods">
      ...
      <form onSubmit={sendMagicLink} className="auth-magic">
        <div className="field">
        ...
        <button type="submit" disabled={status === "sending" || !email}>
          {status === "sending" ? "Wird gesendet …" : "Link zusenden"}
        </button>
      </form>

      {message && (
        <p
          className={status === "error" ? "auth-error" : "auth-success"}
          role="status"
        >
          {message}
        </p>
