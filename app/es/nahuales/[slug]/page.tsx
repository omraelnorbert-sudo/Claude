...
export function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  const description = `${name}: significado, animal de poder y energía de este Nahual en el calendario Tzolk'in MAYA.`;
  return {
    title: name,
    description,
    alternates: { canonical: `/es/nahuales/${params.slug}` },
    openGraph: { title: name, description, url: `/es/nahuales/${params.slug}` },
  };
}

export default async function NahualDetailPageEs({
  params,
}: {
  params: { slug: string };
}) {
  ...
  const content = await getNahualContent(nahualIndex, "es");
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <p style={{ margin: 0, fontSize: 15, color: "var(--text-muted)" }}>
            <strong>Animal de poder:</strong> {description.krafttier}
          </p>
          {content.videoId && (
      ...
      {content.traitText && (
        <section style={{ marginBottom: "var(--space-4)" }}>
          <div className="eyebrow" style={{ marginBottom: 8 }}>
            Para personas con este Nahual de nacimiento
          </div>
